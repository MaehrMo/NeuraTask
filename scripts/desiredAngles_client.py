...
import sys
import rospy
from neura_task.srv import *

def set_desired_angles_client(a1, a2, a3, a4, a5, a6):
    rospy.wait_for_service('set_desired_angles')					#wait for server to open
    try:
...
def usage():										#defines how to use the client
    return "%s a1 a2 a3 a4 a5 a6"%sys.argv[0]

if __name__ == "__main__":
    if len(sys.argv) == 7:
        a1 = int(sys.argv[1])								#write axis values from call in variables
        a2 = int(sys.argv[2])
	a3 = int(sys.argv[3])
        a4 = int(sys.argv[4])
	a5 = int(sys.argv[5])
    ...
    result = set_desired_angles_client(a1, a2, a3, a4, a5, a6)				#call set_desired_angles_client function
